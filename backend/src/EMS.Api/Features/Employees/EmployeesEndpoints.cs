...
public static class EmployeesEndpoints
{
    public static void MapEmployees(this IEndpointRouteBuilder endpoints, string apiVersion)
    {
        ...
        group
          ...
          .WithSummary($"Single Employee");

        group
            .MapPost("/", PostAsync)
            .WithName("CreateEmployee")
            .WithDescription("Create a single new item (Employee)")
            .WithSummary("Create new item (Employee)");

        group
           .MapPatch("/{id}", PatchAsync)
           .WithName("PatchEmployee")
           .WithDescription($"Patches a single existing item (Employee)")
           .WithSummary($"Patches one single item (Employee)");

        group
            .MapDelete("/{id}", DeleteByIdAsync)
            .WithName("DeleteEmployee")
            .WithDescription($"Remove a single Employee.")
            .WithSummary($"Remve an Employee");
